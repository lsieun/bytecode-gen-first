...
public class ConstantPoolGen {
    ...
    private int index = 1; // 索引值，First entry (0) used by JVM
    ...
    private final Map<String, Index> utf8_table = new HashMap(); /*Utf8*/
    private final Map<String, Index> string_table = new HashMap(); /*String*/
    private final Map<String, Index> class_table = new HashMap(); /*Class*/
    private final Map<String, Index> n_a_t_table = new HashMap(); /*NameAndType*/
    private final Map<String, Index> ref_table = new HashMap(); /*Fieldref/Methodref/InterfaceMethodref*/
    ...
    public int lookupFieldref(final String class_name, final String field_name, final String signature) {
        final Index index = ref_table.get(class_name + FIELDREF_DELIM + field_name + FIELDREF_DELIM + signature);
        return (index != null) ? index.index : -1;
    }
    ...
    public int addFieldref(final String class_name, final String field_name, final String signature) {
        ...
        if (!ref_table.containsKey(key)) {
            ref_table.put(key, new Index(ret));
        }
        return ret;
    }
